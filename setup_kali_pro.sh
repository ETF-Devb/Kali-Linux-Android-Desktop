#!/bin/bash
# ==============================================================================
#  KALI LINUX ULTRA-FAST & LIGHTWEIGHT DEPLOYER (ANTI-FREEZE EDITION)
#  Minimal GUI + Kali Menu + Audio + Mousepad ONLY
# ==============================================================================
set -e

clear
echo -e "\033[1;35m==================================================================\033[0m"
echo -e "  \033[1;32mKALI LIGHTWEIGHT DEPLOYER (ULTRA FAST - 0 FREEZE)\033[0m"
echo -e "\033[1;35m==================================================================\033[0m"

# 1. Termux Host Packages
echo -e "\n\033[1;36m[1/5] Preparing Termux Host...\033[0m"
pkg update -y && pkg upgrade -y
pkg install -y wget curl root-repo x11-repo termux-x11-nightly pulseaudio

# 2. NetHunter Installer
echo -e "\n\033[1;36m[2/5] Fetching NetHunter Script...\033[0m"
wget -O install-nethunter-termux https://offs.ec/2MceZWr
chmod +x install-nethunter-termux

echo -e "\n\033[1;33m--------------------------------------------------\033[0m"
echo -e "\033[1;33m IMPORTANT: SELECT OPTION 2 WHEN PROMPTED (Minimal RootFS)\033[0m"
echo -e "\033[1;33m--------------------------------------------------\033[0m"
read -p "Press [ENTER] to launch Installer..."

./install-nethunter-termux

# 3. Inside Kali setup (Bypassing ALL Heavy Triggers)
echo -e "\n\033[1;36m[3/5] Injecting Speed Hacks & Installing Minimal Packages...\033[0m"
nh -r bash << 'INSIDE_KALI'
set -e

# A. DNS Fix
echo "nameserver 8.8.8.8" > /etc/resolv.conf 2>/dev/null || true

# B. SPEED HACK: Kill all heavy triggers that cause hangs/freezes in PRoot
ln -sf /bin/true /usr/bin/mandb 2>/dev/null || true
ln -sf /bin/true /usr/bin/gtk-update-icon-cache 2>/dev/null || true
ln -sf /bin/true /usr/bin/update-desktop-database 2>/dev/null || true
ln -sf /bin/true /usr/bin/update-mime-database 2>/dev/null || true
ln -sf /bin/true /usr/sbin/update-initramfs 2>/dev/null || true

mkdir -p /var/lib/dpkg/info
echo -e "#!/bin/sh\nexit 0" > /var/lib/dpkg/info/systemd.postinst
chmod +x /var/lib/dpkg/info/systemd.postinst

# C. Package Manager Prep
dpkg --configure -a || true
apt update

# D. Install ONLY Essential Desktop + Kali Menu + Audio + 1 Light Tool (Mousepad)
DEBIAN_FRONTEND=noninteractive apt install -y --no-install-recommends \
    wget \
    curl \
    ca-certificates \
    xfce4-session \
    xfwm4 \
    xfce4-panel \
    xfce4-terminal \
    xfce4-whiskermenu-plugin \
    dbus-x11 \
    pulseaudio \
    kali-menu \
    mousepad

# E. Download Single Wallpaper
mkdir -p /usr/share/images/desktop-base
wget -qO /usr/share/images/desktop-base/kali-wallpaper.png https://raw.githubusercontent.com/KaliLinux/kali-wallpapers/main/src/kali-dark-16x9.png || true
ln -sf /usr/share/images/desktop-base/kali-wallpaper.png /etc/alternatives/desktop-background || true

INSIDE_KALI

# 4. Generate Launcher
echo -e "\n\033[1;36m[4/5] Creating Launcher Script...\033[0m"
cat << 'LAUNCHER' > start-gui.sh
#!/bin/bash
pkill -f termux-x11 || true
pkill -f dbus-launch || true
pkill -f xfce4 || true

termux-x11 :1 -listen tcp &
sleep 2
echo -e "\033[1;32m[✔] Server Started! Switch to Termux-X11 App.\033[0m"
nh -r dbus-launch --exit-with-session env DISPLAY=127.0.0.1:1 GALLIUM_DRIVER=llvmpipe xfce4-session
LAUNCHER
chmod +x start-gui.sh

# 5. Finished
echo -e "\n\033[1;32m[✔] DONE! Everything installed cleanly without hangs.\033[0m"
echo -e "Start the desktop anytime with: \033[1;33m./start-gui.sh\033[0m\n"
